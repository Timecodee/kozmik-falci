...
import 'providers/palm_provider.dart';

class PalmScreen extends ConsumerStatefulWidget {
  const PalmScreen({super.key});

  @override
  ConsumerState<PalmScreen> createState() => _PalmScreenState();
}

class _PalmScreenState extends ConsumerState<PalmScreen> {
  ...
  Future<void> _pickImage(ImageSource source) async {
    try {
      ...
      if (pickedFile != null) {
        ...
        if (cropped != null) {
          ref.read(palmProvider.notifier).selectImage(cropped);
        }
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(palmProvider);

    if (state.readingResult != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        final result = state.readingResult!;
        ref.read(palmProvider.notifier).clearImage();
    ...
    return Stack(
      children: [
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      "Avuç İçinizin Fotoğrafı",
                      style: Theme.of(context).textTheme.titleLarge,
                    ),
                    const SizedBox(height: 4),
                    Text(
                      "(Kırpma penceresinden sadece avuç içinizi odaklayarak seçin)",
                        ...
                        child: state.selectedImage == null
                            ? Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 12),
                                  Text(
                                    "Avuç içi resmi seçip kırpmak için dokun",
                                    style: GoogleFonts.lora(
                                      color: AppTheme.goldLight.withOpacity(0.5),
                                      fontSize: 14,
                                    ),
                    ...
                    TextField(
                      ...
                      decoration: InputDecoration(
                        hintText: "Zihnini boşalt, kader çizgilerini okumak için niyetini buraya yaz...",
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: () {
                          ref.read(palmProvider.notifier).getReading(
                                _intentionController.text,
                              );
                        },
                        child: const Text("AVUCUMU ANALİZ ET"),
                      ),
                    ),
                  ],
                ),
        ...
        if (state.isLoading)
          const MysticLoader(
            message: "Avuç içinizdeki kader nehirleri (hayat, akıl, kalp çizgileri) analiz ediliyor...",
          ),
      ],
    );
  }
